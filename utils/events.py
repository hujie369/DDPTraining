...
def write_tblog(tblogger, epoch, results, lrs, loss):
    """Display mAP and loss information to log."""
    tblogger.add_scalar("val/top1", results[0], epoch + 1)
    tblogger.add_scalar("val/top5", results[1], epoch + 1)

    tblogger.add_scalar("train/mean_loss", loss, epoch + 1)
...
def write_tbimg(tblogger, imgs, step, type='train'):
    ...
    if type == 'train':
        ...
        for idx, img in enumerate(imgs):
            tblogger.add_image(
                f'val_img_{idx + 1}', img, step + 1, dataformats='HWC')
    else:
